...
Runs = 1; dMzB = 0; Sigma = 1/2000; %linspace((1/5000),(1/200),Runs);
...
T1_S = 0.965; T1_F = 0.465; T2_S = 0.09; T2_F = 0.012; M0_F = 0.2; k_FS = 8;

% Initialise loop vectors.
T1S_app = zeros(Runs,1); T1F_app = zeros(Runs,1); M0F_app = zeros(Runs,1); kFS_app = zeros(Runs,1); T2S_app = zeros(Runs,1); T2F_app = zeros(Runs,1);
...
Eigenvalues = zeros(6,1,Runs); Ratio = zeros(Runs, 1); Condition_No = zeros(Runs, 1);
...
for ii = 1:Runs
    ...
    %if Exitflag(ii) == 1
        ...
        Condition_No(ii) = Eigenvalues(6,1,ii)/Eigenvalues(1,1,ii);
...
MeanHessian = mean(Hessian,3); MeanEigenvalues = mean(Eigenvalues,3);
MeanCondNo = mean(Condition_No); MeanRatio = mean(Ratio);
