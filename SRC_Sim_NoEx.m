...
T1S_app = zeros(Runs,1); M0F_app = zeros(Runs,1); T2S_app = zeros(Runs,1); T1F_app = zeros(Runs,1); T2F_app = zeros(Runs,1); %Delta_app = zeros(Runs,1);
...
T1S_Sub = zeros(N,1); T1F_Sub = zeros(N,1); T2S_Sub = zeros(N,1); T2F_Sub = zeros(N,1);
M0F_Sub = zeros(N,1); %Delta_Sub = zeros(N,1);
T1S_SRC = zeros(5,1); T1F_SRC = zeros(5,1); T2S_SRC = zeros(5,1); T2F_SRC = zeros(5,1);
M0F_SRC = zeros(5,1); %Delta_SRC = zeros(5,1);
...
for nn = 1:Runs
    ...
    %M0F_LB(1) = 0.001; M0F_UB(1) = 0.35; %Delta_LB(1) = -pi; Delta_UB(1) = pi;
    % Zhang.
    %T1S_LB(1) = 0.7; T1S_UB(1) = 2.5; T2S_LB(1) = 0.075; T2S_UB(1) = 0.2;
    %T1F_LB(1) = 0.2; T1F_UB(1) = 0.5; T2F_LB(1) = 0.002; T2F_UB(1) = 0.045;
    %M0F_LB(1) = 1e-7; M0F_UB(1) = 0.30; %Delta_LB(1) = -pi; Delta_UB(1) = pi;
    % Deoni.
    ...
    for it = 1:Iterations
        ...
        if it < Iterations
            ...
            M0F_LB(it+1) = min(M0F_Sub) ; M0F_UB(it+1) = max(M0F_Sub); %Delta_LB(it+1) = min(Delta_Sub) ; Delta_UB(it+1) = max(Delta_Sub);
    ...
    M0F_app(nn) = mean(M0F_SRC); %Delta_app(nn) = mean(Delta_SRC);
