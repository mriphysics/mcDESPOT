...
int main (int argc, char** argv)
{
  ...
  try {
    for (int n = 1; n < argc; n++) {
      ...
      if (key == "t1f") {spgr.T1_F() = as_double (value);
      ...
      else if (key == "m0s") {spgr.M0_S() = as_double (value);
            		      bssfp.M0_S() = as_double (value);
      			      bssfp180.M0_S() = as_double (value);}
      else if (key == "kfs") {spgr.k_FS() = as_double (value);
      			      bssfp.k_FS() = as_double (value);
      			      bssfp180.k_FS() = as_double (value);}
      else if (key == "ksf") {spgr.k_SF() = as_double (value);
            		      bssfp.k_SF() = as_double (value);
      			      bssfp180.k_SF() = as_double (value);}
        ...
        std::istringstream fassfp0_stream (value);
        std::vector<double> fassfp0;
        do {
          double d;
          fassfp0_stream >> d;
          if (!fassfp0_stream.good())
            break;
          fassfp0.push_back (d * 3.14159265358979323846 / 180.0);
        } while (fassfp0_stream.good());
        bssfp.FA.resize (fassfp0.size());
        for (int n = 0; n < fassfp0.size(); n++)
          bssfp.FA[n] = fassfp0[n];
      }

      else if (key == "fassfp180") {
        std::istringstream fassfp180_stream (value);
        std::vector<double> fassfp180;
        do {
          double d;
          fassfp180_stream >> d;
          if (!fassfp180_stream.good())
            break;
          fassfp180.push_back (d * 3.14159265358979323846 / 180.0);
        } while (fassfp180_stream.good());
        bssfp180.FA.resize (fassfp180.size());
        for (int n = 0; n < fassfp180.size(); n++)
          bssfp180.FA[n] = fassfp180[n];
      }
