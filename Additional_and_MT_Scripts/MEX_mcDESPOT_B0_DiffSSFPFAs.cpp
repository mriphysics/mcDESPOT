/*
 * MEX_mcDESPOT_B0_DiffSSFPFAs.cpp.
 *
 * Takes input of T1F, T1S, T2F, T2S, kFS, kSF, M0F, M0S, TRs, Delta, FAs and 
...
void MEX_mcDESPOT(double t1f, double t1s, double t2f, double t2s, double kfs, double ksf, double m0f, double m0s, double trspgr, double trssfp, double delta, double *faspgr, double *fassfp0, double *fassfp180, double *SPGR_Sig, double *bSSFP0_Sig, double *bSSFP180_Sig, mwSize n_faspgr, mwSize n_fassfp0, mwSize n_fassfp180)

{
  ...
  spgr.k_SF() = ksf; bssfp.k_SF() = ksf; bssfp180.k_SF() = ksf;
  
  spgr.M0_F() = m0f; bssfp.M0_F() = m0f; bssfp180.M0_F() = m0f;
  spgr.M0_S() = m0s; bssfp.M0_S() = m0s; bssfp180.M0_S() = m0s;
...
{
    double RT1, RT2, RT3, RT4, ER1, ER2, FM1, FM2, SPGR_TR, SSFP_TR, B0;
    double *SPGR_FA;                                                          
    size_t ncols_spgr;
    double *SSFP0_FA;
    size_t ncols_ssfp0;
    ...
    ER2 = mxGetScalar(prhs[5]);
    FM1 = mxGetScalar(prhs[6]);
    FM2 = mxGetScalar(prhs[7]);
    SPGR_TR = mxGetScalar(prhs[8]);
    SSFP_TR = mxGetScalar(prhs[9]);
    B0 = mxGetScalar(prhs[10]);

    /* Create a pointer to the real data in the input matrices.  */
    SPGR_FA = mxGetPr(prhs[11]);
    SSFP0_FA = mxGetPr(prhs[12]);
    SSFP180_FA = mxGetPr(prhs[13]);

    /* Get dimensions of the input matrices. */
    ncols_spgr = mxGetN(prhs[11]);
    ncols_ssfp0 = mxGetN(prhs[12]);
    ncols_ssfp180 = mxGetN(prhs[13]);
    ...
    MEX_mcDESPOT(RT1,RT2,RT3,RT4,ER1,ER2,FM1,FM2,SPGR_TR,SSFP_TR,B0,SPGR_FA,SSFP0_FA,SSFP180_FA,SPGR,bSSFP0,bSSFP180,(mwSize)ncols_spgr,(mwSize)ncols_ssfp0,(mwSize)ncols_ssfp180);
