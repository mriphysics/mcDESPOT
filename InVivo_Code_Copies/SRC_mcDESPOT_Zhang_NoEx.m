%%% SRC for in vivo data. Model EXCLUDES exchange. %%%

function [T1S_final, T1F_final, T2S_final, T2F_final, M0F_final, Delta_final] = SRC_mcDESPOT_Zhang_NoEx(Coords, Trials, Iterations, N, FA_SPGR, FA_SSFP180, FA_SSFP0, TR_SPGR, TR_SSFP, Data)

T1S_final = zeros(length(Coords),1); T2S_final = zeros(length(Coords),1); M0F_final = zeros(length(Coords),1); T1F_final = zeros(length(Coords),1); T2F_final = zeros(length(Coords),1); Delta_final = zeros(length(Coords),1);
...
parfor mm = 1:length(Coords)
    ...
    SRSQ = zeros(Trials,1);
    T1S_Sub = zeros(N,1); T2S_Sub = zeros(N,1); M0F_Sub = zeros(N,1); T1F_Sub = zeros(N,1); T2F_Sub = zeros(N,1); Delta_Sub = zeros(N,1);
    T1S_SRC = zeros(5,1); T2S_SRC = zeros(5,1); M0F_SRC = zeros(5,1); T1F_SRC = zeros(5,1); T2F_SRC = zeros(5,1); Delta_SRC = zeros(5,1);
    ...
    for it = 1:Iterations
        ...
        if it == 1
            ...
            M0F_Pick = (M0F_UB(it) - M0F_LB(it)) .* rand(Trials,1) + M0F_LB(it); Delta_Pick = (Delta_UB(it) - Delta_LB(it)) .* rand(Trials,1) + Delta_LB(it);
        ...
        for qq = 1:Trials
            ...
            [Mss_SPGR(:,qq), Mss_SSFP0(:,qq), Mss_SSFP180(:,qq)] = MEX_mcDESPOT_B0_DE_DiffFAs(T1F_Pick(qq), T1S_Pick(qq), T2F_Pick(qq), T2S_Pick(qq), 0, M0F_Pick(qq), TR_SPGR, TR_SSFP, Delta_Pick(qq), FA_SPGR(mm,:), FA_SSFP0(mm,:), FA_SSFP180(mm,:));
        ...
        if it < Iterations
            ...
            for pp = 1:N
                ...
                M0F_Sub(pp) = M0F_Pick(Index(pp)); Delta_Sub(pp) = Delta_Pick(Index(pp));
            ...
            M0F_LB(it+1) = min(M0F_Sub) ; M0F_UB(it+1) = max(M0F_Sub); T2S_LB(it+1) = min(T2S_Sub) ; T2S_UB(it+1) = max(T2S_Sub); 
            T2F_LB(it+1) = min(T2F_Sub) ; T2F_UB(it+1) = max(T2F_Sub); Delta_LB(it+1) = min(Delta_Sub); Delta_UB(it+1) = max(Delta_Sub);
            ...
            M0F_expn = (M0F_UB(it+1) - M0F_LB(it+1))/N; Delta_expn = (Delta_UB(it+1) - Delta_LB(it+1))/N;
            ...
            if (((max(T1S_Sub)-min(T1S_Sub))/max(T1S_Sub)) < 0.01 && ((max(T1F_Sub)-min(T1F_Sub))/max(T1F_Sub)) < 0.01 && ((max(M0F_Sub)-min(M0F_Sub))/max(M0F_Sub)) < 0.01 && ((max(T2S_Sub)-min(T2S_Sub))/max(T2S_Sub)) < 0.01 && ((max(T2F_Sub)-min(T2F_Sub))/max(T2F_Sub)) < 0.01 && ((max(Delta_Sub)-min(Delta_Sub))/max(Delta_Sub)) < 0.01)
                break
            end
    ...
    for pp = 1:5
        ...
        M0F_SRC(pp) = M0F_Pick(Index(pp)); Delta_SRC(pp) = Delta_Pick(Index(pp));
    end
    
    T1S_final(mm) = mean(T1S_SRC); T2S_final(mm) = mean(T2S_SRC); M0F_final(mm) = mean(M0F_SRC); T1F_final(mm) = mean(T1F_SRC); T2F_final(mm) = mean(T2F_SRC); Delta_final(mm) = mean(Delta_SRC);
