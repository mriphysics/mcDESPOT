...
function [Sol, Bounds, Subsets] = SRC_SimTest(Trials, Iterations, N, Runs, FA_SPGR, FA_SSFP0, FA_SSFP180, TR_SPGR, TR_SSFP, Data) %FA_SSFP0, FA_SSFP90, FA_SSFP270,
% Fixed_Param as 1st input.
...
for nn = 1:Runs
    ...
    Delta_LB(1) = -2*pi; Delta_UB(1) = 2*pi;
    % Bouhrara Bounds + k.
    ...
    for it = 1:Iterations
        ...
        for qq = 1:Trials
            ...
            [Mss_SPGR(:,qq), Mss_SSFP(:,qq), Mss_SSFP180(:,qq)] = MEX_mcDESPOT_B0_DE_DiffFAs(T1F_Pick(qq), T1S_Pick(qq), T2F_Pick(qq), T2S_Pick(qq), kFS_Pick(qq), M0F_Pick(qq), TR_SPGR, TR_SSFP, Delta_Pick(qq), rad2deg(FA_SPGR), rad2deg(FA_SSFP0), rad2deg(FA_SSFP180));
