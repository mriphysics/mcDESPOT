...
function [Sol, Bounds, Subsets] = SRC_Sim_NDE_v2(Trials, Iterations, N, Runs, FA_SPGR, FA_SSFP0, FA_SSFP180, TR_SPGR, TR_SSFP, Data)
Bounds = zeros(Iterations,16); Subsets = zeros(5,8);
...
M0S_app = zeros(Runs,1); kSF_app = zeros(Runs,1);
...
M0S_Sub = zeros(N,1); kSF_Sub = zeros(N,1);
M0S_SRC = zeros(5,1); kSF_SRC = zeros(5,1);
...
M0S_LB = zeros(Iterations,1); M0S_UB = zeros(Iterations,1);
kSF_LB = zeros(Iterations,1); kSF_UB = zeros(Iterations,1);
...
for nn = 1:Runs
    ...
    M0S_LB(1) = 0; M0S_UB(1) = 1.0; kSF_LB(1) = 0.5; kSF_UB(1) = 20;
    ...
    for it = 1:Iterations
        ...
        for qq = 1:Trials
            ...
            %Mss_SPGR(:,qq) = SPGR_SteadyState_nonDE(FA_SPGR,TR_SPGR,'T1_S',T1S_Pick(qq),'T1_F',T1F_Pick(qq),'M0_F',M0F_Pick(qq),'M0_S',M0S_Pick(qq),'k_FS',kFS_Pick(qq),'k_SF',kSF_Pick(qq));
            %Mss_SSFP(:,qq) = SSFP_SteadyState_nonDE(FA_SSFP0,TR_SSFP,0,'T1_S',T1S_Pick(qq),'T2_S',T2S_Pick(qq),'T1_F',T1F_Pick(qq),'T2_F',T2F_Pick(qq),'M0_F',M0F_Pick(qq),'M0_S',M0S_Pick(qq),'k_FS',kFS_Pick(qq),'k_SF',kSF_Pick(qq));
            %Mss_SSFP180(:,qq) = SSFP_SteadyState_nonDE(FA_SSFP180,TR_SSFP,pi,'T1_S',T1S_Pick(qq),'T2_S',T2S_Pick(qq),'T1_F',T1F_Pick(qq),'T2_F',T2F_Pick(qq),'M0_F',M0F_Pick(qq),'M0_S',M0S_Pick(qq),'k_FS',kFS_Pick(qq),'k_SF',kSF_Pick(qq));
           
            [Mss_SPGR(:,qq), Mss_SSFP(:,qq), Mss_SSFP180(:,qq)] = MEX_mcDESPOT_B0_DiffSSFPFAs(T1F_Pick(qq), T1S_Pick(qq), T2F_Pick(qq), T2S_Pick(qq), kFS_Pick(qq), kSF_Pick(qq), M0F_Pick(qq), M0S_Pick(qq), TR_SPGR, TR_SSFP, 0, rad2deg(FA_SPGR), rad2deg(FA_SSFP0), rad2deg(FA_SSFP180));
        ...
        if it < Iterations
            ...
            M0S_LB(it+1) = min(M0S_Sub) ; M0S_UB(it+1) = max(M0S_Sub); kSF_LB(it+1) = min(kSF_Sub) ; kSF_UB(it+1) = max(kSF_Sub);
            ...
            if (((max(T1S_Sub)-min(T1S_Sub))/max(T1S_Sub)) < 0.01 && ((max(T1F_Sub)-min(T1F_Sub))/max(T1F_Sub)) < 0.01 && ((max(M0F_Sub)-min(M0F_Sub))/max(M0F_Sub)) < 0.01 && ((max(T2S_Sub)-min(T2S_Sub))/max(T2S_Sub)) < 0.01 && ((max(T2F_Sub)-min(T2F_Sub))/max(T2F_Sub)) < 0.01 && ((max(kFS_Sub)-min(kFS_Sub))/max(kFS_Sub)) < 0.01 && ((max(M0S_Sub)-min(M0S_Sub))/max(M0S_Sub)) < 0.01 && ((max(kSF_Sub)-min(kSF_Sub))/max(kSF_Sub)) < 0.01)
               break
            end
    ...
    for pp = 1:5
        ...
        M0S_SRC(pp) = M0S_Pick(Index(pp)); kSF_SRC(pp) = kSF_Pick(Index(pp));
    end
    ...
    M0S_app(nn) = mean(M0S_SRC); kSF_app(nn) = mean(kSF_SRC);
    
end

Sol(:,:) = [T1S_app(:), T1F_app(:), T2S_app(:), T2F_app(:), M0F_app(:), M0S_app(:), kFS_app(:), kSF_app(:)];
Bounds(:,:) = [T1S_LB(:), T1S_UB(:), T1F_LB(:), T1F_UB(:), T2S_LB(:), T2S_UB(:), T2F_LB(:), T2F_UB(:), M0F_LB(:), M0F_UB(:), M0S_LB(:), M0S_UB(:), kFS_LB(:), kFS_UB(:), kSF_LB(:), kSF_UB(:)];
Subsets(:,:) = [T1S_SRC, T1F_SRC, T2S_SRC, T2F_SRC, M0F_SRC, kFS_SRC, M0S_SRC, kSF_SRC];
