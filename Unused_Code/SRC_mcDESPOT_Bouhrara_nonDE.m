...
function [T1S_final, T1F_final, T2S_final, T2F_final, kFS_final, kSF_final, M0F_final, M0S_final, Delta_final] = SRC_mcDESPOT_Bouhrara_nonDE(Coords, Trials, Iterations, N, FA_SPGR, FA_SSFP180, FA_SSFP0, TR_SPGR, TR_SSFP, Data)
...
M0S_final = zeros(length(Coords),1); kSF_final = zeros(length(Coords),1);
...
parfor mm = 1:length(Coords)
    ...
    for it = 1:Iterations
        ...
        for qq = 1:Trials
            ...
            [Mss_SPGR(:,qq), Mss_SSFP0(:,qq), Mss_SSFP180(:,qq)] = MEX_mcDESPOT_B0_DiffSSFPFAs(T1F_Pick(qq), T1S_Pick(qq), T2F_Pick(qq), T2S_Pick(qq), kFS_Pick(qq), kSF_Pick(qq), M0F_Pick(qq), M0S_Pick(qq), TR_SPGR, TR_SSFP, Delta_Pick(qq), FA_SPGR(mm,:), FA_SSFP0(mm,:), FA_SSFP180(mm,:));
        ...
        if it < Iterations
            ...
            if (((max(T1S_Sub)-min(T1S_Sub))/max(T1S_Sub)) < 0.01 && ((max(T1F_Sub)-min(T1F_Sub))/max(T1F_Sub)) < 0.01 && ((max(M0F_Sub)-min(M0F_Sub))/max(M0F_Sub)) < 0.01 && ((max(kFS_Sub)-min(kFS_Sub))/max(kFS_Sub)) < 0.01 && ((max(T2S_Sub)-min(T2S_Sub))/max(T2S_Sub)) < 0.01 && ((max(T2F_Sub)-min(T2F_Sub))/max(T2F_Sub)) < 0.01 && ((max(Delta_Sub)-min(Delta_Sub))/max(Delta_Sub)) < 0.01 && ((max(M0S_Sub)-min(M0S_Sub))/max(M0S_Sub)) < 0.01 && ((max(kSF_Sub)-min(kSF_Sub))/max(kSF_Sub)) < 0.01)
                break
            end
    ...
    M0S_final(mm) = mean(M0S_SRC); kSF_final(mm) = mean(kSF_SRC);
