...
% k_FS = 9; k_SB = 5; k_FB = 5; k_SF = (M0_F*k_FS)/M0_S; % Liu.

TR_SPGR = 5e-3; TR_SSFP = 5e-3;
FA_SPGR = deg2rad([3 4 5 6 7 9 13 18]); 
FA_SSFP = deg2rad([10 13 17 20 23 30 43 60]);
T1_S = 0.965; T1_F = 0.465; T1_B = 1; 
T2_S = 0.09; T2_F = 0.012;   
M0_F = 0.2; M0_S = 0.7; M0_B = 0.1;
k_FS = 8; k_SB = 5; k_FB = 5; k_SF = (M0_F*k_FS)/M0_S; % Deoni.

R1_F = 1/T1_F; R1_B = 1/T1_B; R1_S = 1/T1_S; dMzB = 0;
G = 1.4e-5; Gamma = 2 * pi * 42.57747892e6; B1 = 13e-6;
T_RF = deg2rad(50)/(Gamma * B1); W = (pi/T_RF) * (Gamma * B1)^2 * T_RF * G * (T_RF/TR_SSFP);
...
% GT-signals - change between B1 and TRF.
SPGR_Data = B1_SPGR_steady_state_MT_M0(FA_SPGR,TR_SPGR,'T1_S',T1_S,'T1_F',T1_F,'T1_B',T1_B,'M0_B',M0_B,'M0_F',M0_F,'M0_S',M0_S,'k_FS',k_FS,'k_SB',k_SB,'k_FB',k_FB);
SSFP_Data = B1_SSFP_steady_state_MT_M0(FA_SSFP,TR_SSFP,'T1_S',T1_S,'T2_S',T2_S,'T1_F',T1_F,'T2_F',T2_F,'T1_B',T1_B,'M0_B',M0_B,'M0_F',M0_F,'M0_S',M0_S,'k_FS',k_FS,'k_SB',k_SB,'k_FB',k_FB);
SSFP_Data_180 = B1_SSFP_steady_state_180_MT_M0(FA_SSFP,TR_SSFP,'T1_S',T1_S,'T2_S',T2_S,'T1_F',T1_F,'T2_F',T2_F,'T1_B',T1_B,'M0_B',M0_B,'M0_F',M0_F,'M0_S',M0_S,'k_FS',k_FS,'k_SB',k_SB,'k_FB',k_FB);
...
Upper = [1.25 0.5 0.3 0.8 20 10 0.15 0.03];
Lower = [0.2 0.1 0.05 0.05 1 1 0.04 0.01];

% Upper = [3 1.5 1 1 40 40 0.5 0.1];
% Lower = [0.01 0.01 0.01 0.01 0.01 0.01 0.01 0.01];
